    ...
    async function fetchStats() {
      try {
        ...
        setStats({
          ...
          totalPoints,
          pendingPicks: Math.max(0, (leagues?.length || 0) * 2 - (picks?.length || 0)),
        })
      } catch (error) {
  ...
  if (loading) {
    return (
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[...Array(4)].map((_, i) => (
          <Card key={i} className="backdrop-blur-sm bg-card/80 border-border/50">
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
      {statCards.map((stat, index) => (
        <Card key={index} className="backdrop-blur-sm bg-card/80 border-border/50 hover:bg-card/90 transition-all">
          <CardContent className="p-6">
