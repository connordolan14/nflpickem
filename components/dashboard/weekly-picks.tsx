...
export function WeeklyPicks({ userId }: WeeklyPicksProps) {
    ...
    async function fetchWeeklyPicks() {
      try {
        ...
        const statuses: PickStatus[] =
          leagues?.map((league) => {
            const leaguePicks = picks?.filter((pick) => pick.league_id === league.league_id) || []
            return {
  ...
  const pendingPicks = pickStatuses.filter((status) => status.picks_made < status.picks_needed)
  const completedPicks = pickStatuses.filter((status) => status.picks_made >= status.picks_needed)
      ...
      <CardContent>
        {pickStatuses.length === 0 ? (
          <div className="text-center py-6">
            ...
            <p className="text-sm text-muted-foreground">No leagues to make picks for.</p>
          </div>
        ) : (
          <div className="space-y-3">
            ...
            {pendingPicks.map((status) => (
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <h4 className="font-medium text-foreground">{status.league_name}</h4>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Button asChild size="sm" variant="outline">
                    <Link href={`/leagues/${status.league_id}/picks`}>Make Picks</Link>
                  </Button>
                </div>
              </div>
            ))}
            ...
            {completedPicks.map((status) => (
              ...
              >
                ...
                <div className="flex items-center justify-between">
                  <div className="text-xs text-muted-foreground">Picks submitted for Week {currentWeek}</div>
                  <Button asChild size="sm" variant="ghost">
                    <Link href={`/leagues/${status.league_id}/picks`}>View Picks</Link>
                  </Button>
                </div>
              </div>
            ))}
